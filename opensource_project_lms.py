...
# 셀레니움을 통해 chrome창을 열 때 사용할 옵션들, 열리는 창의 크기 설정 및 창의 자동으로 닫힘 방지한다
options = Options()
...
# 창이 열리기까지 시간이 조금 걸리므로 2초 기다려서 안정성을 높인다. 이후 time.sleep도 동일하다
time.sleep(2)
...
#w모드 (쓰기 모드) 로 전에 설정한 경로 및 이름으로 파일을 열어서 헤더를 위에 작성한다
f = open(file_name, "w")
f.write(file_header)
...
#j 초기값 1 설정 (몇번째 인지 나타내는 번호를 의미 하는 j 변수) 한다
j = 1
